    ...
    const userArr = userList.map((user) => {
        if (user.name === "Admin") {
            return (
                <tr
                    key={user.email}
                    className=" border-b hover:bg-gray-800 bg-red-500 dark:hover:bg-gray-600"
                >
                    <th
                        scope="row"
                        className="px-2 py-4 font-medium text-white dark:text-white whitespace-nowrap"
                    >
                        {user.name}
                    </th>
        ...
        return (
            ...
            >
                ...
                <td id={user._id} className="px-2 py-4 text-white">
                    <IoMdTrash
                        id={user._id}
                        onClick={(e) => {
                            setShow(true);
                    ...
                    <ul className="list-reset flex flex-row md:flex-col text-center md:text-left mt-10">
                        ...
                        <li className="mr-3 flex-1">
                            <a
                                ...
                                className="flex py-1 md:py-3 pl-1 align-middle text-gray-800 no-underline cursor-pointer hover:text-pink-500 border-b-2 border-gray-800 md:border-gray-900 hover:border-pink-500"
                            >
...
export async function getStaticProps() {
    const response = await axios.get(`${process.env.NEXT_PUBLIC_API_FETCH}/user`);
    return {
        props: {
